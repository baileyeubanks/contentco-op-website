...
export default function CoProofHome() {
  const [assets, setAssets] = useState<Asset[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("/api/coproof/assets")
      .then((r) => r.json())
    ...
    <main className="shell">
      <header className="nav">
        ...
        <nav className="nav-links">
          ...
          <a className="active" href="#">Co-Proof</a>
        </nav>
      ...
      <section className="grid" style={{ marginTop: "1rem" }}>
        ...
        <article className="panel">
          <div className="kicker">Proof Queue</div>
          <h1 style={{ fontSize: "3.2rem" }}>Delivery proofing</h1>
          ...
          {loading ? (
            ...
            <div style={{ marginTop: "1rem", padding: "1.5rem", textAlign: "center", border: "1px solid #2b4263", borderRadius: 14, background: "#0a1524" }}>
              <p className="muted">No deliverables yet. Assets created in Co-Edit will appear here for proofing.</p>
            </div>
          ) : (
            <div className="table">
              <div className="row">
