...
export default function AssetReview() {
  ...
  useEffect(() => {
    if (!id) return;
    fetch(`/api/coproof/assets/${id}`).then((r) => r.json()).then(setAsset).catch(() => {});
    fetch(`/api/coproof/assets/${id}/comments?asset_id=${id}`).then((r) => r.json()).then((d) => setComments(d.items ?? [])).catch(() => {});
    fetch(`/api/coproof/assets/${id}/gates`).then((r) => r.json()).then((d) => setGates(d.items ?? [])).catch(() => {});
    fetch(`/api/coproof/assets/${id}/versions`).then((r) => r.json()).then((d) => setVersions(d.items ?? [])).catch(() => {});
    fetch(`/api/coproof/assets/${id}/audit-log`).then((r) => r.json()).then((d) => setEvents(d.items ?? [])).catch(() => {});
  }, [id]);

  async function addComment() {
    if (!newComment.trim() || !newTimecode.trim()) return;
    setSubmitting(true);
    const res = await fetch("/api/coproof/assets/" + id + "/comments", {
      method: "POST",
  ...
  async function resolveComment(commentId: string) {
    const res = await fetch(`/api/coproof/assets/${id}/comments/${commentId}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  async function submitDecision(gateId: string, decision: string) {
    ...
    const res = await fetch(`/api/coproof/approvals/${gateId}/decision`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
