...
function ContactsPageInner() {
  ...
  const filtered = useMemo(() => {
    let c = allContacts;
    if (buFilter !== "ALL") c = c.filter((r) => (r.business_unit || "ACS").toUpperCase() === buFilter);
    if (searchQ) {
  ...
  const acsCount = allContacts.filter((c) => (c.business_unit || "ACS").toUpperCase() === "ACS").length;
  const ccCount = allContacts.filter((c) => (c.business_unit || "ACS").toUpperCase() === "CC").length;
  const now = new Date();
