...
function buildDraftFromBrief(brief: Record<string, unknown>) {
  const deliverables = parseList(String(brief.deliverables || ""));
  const references = parseList(String(brief.references || ""));
  const keyMessages = parseList(String(brief.key_messages || ""));
  const base = estimateBase(String(brief.content_type || ""));
  const deliverableBonus = Math.max(0, deliverables.length - 1) * 450;
  const rushDays = daysUntil(String(brief.deadline || ""));
  const rushMultiplier = rushDays != null && rushDays <= 14 ? 1.2 : 1;
  ...
  return {
    ...
    doc: {
      ...
      project: {
        ...
        meta: [brief.content_type, brief.location, brief.deadline].filter(Boolean),
        summary: [brief.objective, brief.audience].filter(Boolean).map((value) => String(value)),
      },
      bill_to: {
...
export async function POST(_req: Request, { params }: Props) {
  ...
  const contact = await findOrCreateCcContact(brief, business.id);
