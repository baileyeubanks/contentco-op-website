...
type RootAuthCacheEntry = {
  path: string;
  mtimeMs: number;
  loadedAtMs: number;
  config: RootAuthFileConfig | null;
};
...
let rootAuthCache: RootAuthCacheEntry | null = null;
...
function readRootAuthFile(): RootAuthFileConfig | null {
  for (const candidate of getRootAuthConfigPathCandidates()) {
    try {
      ...
      if (
        rootAuthCache &&
        rootAuthCache.path === candidate &&
        rootAuthCache.mtimeMs === stat.mtimeMs &&
        now - rootAuthCache.loadedAtMs < ROOT_AUTH_CACHE_TTL_MS
      ) {
        return rootAuthCache.config;
      }
