...
async function executeAction(action: AutomationAction, context: {
  ...
  switch (action.type) {
    ...
    case "send_email": {
      const to = String(action.config.to || context.payload.client_email || "");
      const subject = String(action.config.subject || `Notification: ${context.eventType}`);
      const body = String(action.config.body || action.config.html || `<p>An event occurred: ${context.eventType}</p>`);

      if (!to) return { ok: false, error: "Missing email recipient (to)" };
      ...
      return result.ok
        ? { ok: true, result: { email_id: result.id } }
        : { ok: false, error: result.error };
    }

    case "notify_slack": {
      const webhookUrl = String(action.config.webhook_url || process.env.SLACK_WEBHOOK_URL || "");
      if (!webhookUrl) return { ok: false, error: "Missing Slack webhook URL" };

      const text = String(action.config.text || `[${context.businessUnit}] ${context.eventType}`);
      try {
        ...
        return { ok: false, error: error instanceof Error ? error.message : "Slack notify failed" };
      }
    }
