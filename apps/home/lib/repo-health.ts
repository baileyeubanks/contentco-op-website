...
function resolvePublicAssetPath(src: string) {
  ...
  const contentRelativePath = relativePath.startsWith("cc/") ? relativePath.slice(3) : relativePath;
  return [
    path.join(process.cwd(), "public", relativePath),
    path.join(process.cwd(), "apps/home/public", relativePath),
    path.join(process.cwd(), "media", relativePath),
    path.join(process.cwd(), "apps/home/media", relativePath),
    path.join(process.cwd(), "media", contentRelativePath),
    path.join(process.cwd(), "apps/home/media", contentRelativePath),
    path.join(process.cwd(), "content", relativePath),
    path.join(process.cwd(), "apps/home/content", relativePath),
    path.join(process.cwd(), "content", contentRelativePath),
    path.join(process.cwd(), "apps/home/content", contentRelativePath),
  ];
}
...
async function buildPortfolioChecks(scope: RepoHealthScope): Promise<RepoHealthCheck[]> {
  ...
  for (const study of portfolioStudies) {
    ...
    if (scope === "full") {
      ...
      if (study.review?.status !== "approved") {
        reviewWarnings.push(`${study.id}: review status is ${study.review?.status || "unknown"}`);
      }
    ...
    if (scope === "full" && study.remoteMediaUrl) {
      try {
  ...
  return [
    ...
    },
    {
      ...
      detail: missingAssets.length
        ? `Missing assets: ${Array.from(new Set(missingAssets)).join(", ")}`
        : reviewWarnings.length
          ? reviewWarnings.join("; ")
          : "All referenced local assets exist and review metadata is current.",
      updatedAt: now(),
      meta: {
        missingAssetCount: new Set(missingAssets).size,
...
async function buildRuntimeCheck(scope: RepoHealthScope): Promise<RepoHealthCheck> {
  ...
  const shouldWarnOptional = scope === "full" && missingOptional.length > 0;

  return {
    id: "runtime_env",
    label: "Runtime environment",
    status: missingRequired.length ? "fail" : shouldWarnOptional ? "warn" : "ok",
...
export async function getRepoHealthSnapshot(scope: RepoHealthScope = "full"): Promise<RepoHealthSnapshot> {
  const checks = [
    ...(await buildPortfolioChecks(scope)),
    await buildRouteCheck(),
    await buildRuntimeCheck(scope),
  ...
  return {
    ...
    generatedAt: now(),
    version: process.env.COMMIT_SHA || process.env.BUILD_COMMIT_SHA || portfolioManifest.version,
    summary,
    checks,
  };
}
