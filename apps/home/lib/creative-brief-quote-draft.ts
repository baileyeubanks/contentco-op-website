...
export async function withSubmittedBriefPayloadFallback(brief: Record<string, unknown>) {
...
function buildDraftFromBrief(brief: Record<string, unknown>) {
  ...
  const projectTitle = String(brief.role || brief.company || brief.contact_name || "Content Co-op Project").trim();
...
export async function createQuoteDraftFromBriefId(briefId: string) {
  ...
  if (error || !brief) {
    throw new Error(error?.message || "brief_not_found");
  }
  ...
  if (!business?.id) {
    throw new Error("content_co_op_business_missing");
  }
  ...
  const { data: quote, error: quoteError } = await supabase
    ...
    .select("id, quote_number, estimated_total, status, internal_status, client_status, client_name, business_unit, payload, issue_date, valid_until, payment_terms, client_email")
    .single();

  if (quoteError || !quote) {
    throw new Error(quoteError?.message || "quote_insert_failed");
  }
  ...
  const hermes = await emitBlazeHandoff({
    businessUnit: "CC",
    channel: "website",
    sourceSystem: "cco_quote_draft",
    identityHints: {
      brief_id: brief.id,
      external_id: quote.id,
      email: String(brief.contact_email || "").trim() || null,
...
export async function renderQuotePdfForQuoteId(quoteId: string) {
  ...
  if (error || !quote) {
    throw new Error(error?.message || "quote_not_found");
  }
  ...
  if (itemsError) {
    throw new Error(itemsError.message || "quote_items_not_found");
  }
  ...
  const filename = `Quote_${quote.quote_number || quoteId}_${String(quote.client_name || "draft").replace(/\s+/g, "_")}.pdf`;
  return {
