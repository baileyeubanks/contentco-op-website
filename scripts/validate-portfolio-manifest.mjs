...
function resolvePortfolioAssetPath(src) {
  const normalized = String(src || "");
  return resolvePublicAssetPath(normalized);
}

async function probeDurationSeconds(src) {
  if (!src || !String(src).startsWith("/")) return null;
  const assetPath = resolvePortfolioAssetPath(src);
...
export async function runPortfolioValidation() {
  ...
  for (const entry of manifest.entries || []) {
    ...
    for (const image of entry.gallery) {
      ...
      if (!image.alignment) {
        reviewWarnings.push(`${entry.id}: gallery alignment missing for ${image.src}`);
      } else if (!["client_specific", "shared_environment"].includes(image.alignment)) {
        manifestIssues.push(`${entry.id}: invalid alignment for ${image.src}`);
      }
      if (String(image.src || "").startsWith("/")) {
        const exists = await fileExists(resolvePortfolioAssetPath(image.src));
        if (!exists) assetIssues.push(`${entry.id}: missing asset ${image.src}`);
      }
    }

    const previewDuration = await probeDurationSeconds(entry.preview);
    const videoDuration = await probeDurationSeconds(entry.video);

    if (entry.preview && entry.video && entry.preview === entry.video) {
      reviewWarnings.push(`${entry.id}: preview and master share the same file`);
    }
    if (previewDuration !== null && videoDuration !== null && previewDuration >= videoDuration * 0.95) {
      reviewWarnings.push(
        `${entry.id}: preview duration (${previewDuration.toFixed(1)}s) is nearly the same as master (${videoDuration.toFixed(1)}s)`,
      );
    }
    if (videoDuration !== null && videoDuration < 10) {
      reviewWarnings.push(`${entry.id}: master video is only ${videoDuration.toFixed(1)}s`);
    }
    if (videoDuration !== null && videoDuration < 15 && /film|campaign|documentary/i.test(entry.format || "")) {
      reviewWarnings.push(`${entry.id}: master is ${videoDuration.toFixed(1)}s but format is "${entry.format}" — may be a preview clip mislabeled as master`);
    }
