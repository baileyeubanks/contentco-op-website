...
async function main() {
  ...
  try {
    ...
    throw new Error(`failed to parse platform manifest from ${manifestUrl}: ${error.message}`);
  }
  ...
  for (const routeId of smokeRouteIds) {
    ...
    try {
      const redirect = route.id === "booking" ? "manual" : "follow";
      // eslint-disable-next-line no-await-in-loop
      const { response, text } = await fetchText(url, redirect);
      let ok = okStatus(response);
      if (route.id === "booking") {
        ok = response.status >= 300 && response.status < 400 && /calendar\.google\.com/.test(response.headers.get("location") || "");
